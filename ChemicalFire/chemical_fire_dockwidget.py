...
########################################################################################################################
    ...
    def buildLayerVeryToxic(self):
        ...
        if not verytoxic_building:
            ...
            extent = verytoxic_building.extent()
        ...
        if not mediumtoxic_building:
            ...
            mediumtoxic_building.updateExtents()
            extent = mediumtoxic_building.extent()
        ...
        delete_nottoxic_building = uf.getLegendLayerByName(self.iface, "NotToxic")
        if delete_nottoxic_building:
            QgsMapLayerRegistry.instance().removeMapLayers([delete_nottoxic_building.id()])
        nottoxic_building = uf.getLegendLayerByName(self.iface, "NotToxic")
        level1 = 1
        featuresnottoxic = uf.getFeaturesByExpression(risk_buildings,'"RiskLevel" = %s' %level1)
        if not nottoxic_building:
            # retrieve buildings inside buffer (inside = True)
            # add these buildings to a new temporary layer
            fields_names = [field.name() for field in self.buildings_layer.dataProvider().fields()]
            fields_types = [field.type() for field in self.buildings_layer.dataProvider().fields()]
            nottoxic_building = uf.createTempLayer('NotToxic','POLYGON',risk_buildings.crs().postgisSrid(),fields_names, fields_types)
            nottoxic_building.dataProvider().addFeatures(featuresnottoxic)
            nottoxic_building.updateExtents()
            extent = nottoxic_building.extent()
            uf.loadTempLayer(nottoxic_building)
            legend = self.iface.legendInterface()
            legend.setLayerVisible(nottoxic_building, False)
            self.canvas.setExtent(extent)
            self.canvas.refresh()
            self.canvas.zoomOut()
        self.infoVeryToxic()
    ...
    def infoVeryToxic(self):
        ...
        # change "risk" to "name of risk field"
        nottoxic_building = uf.getLegendLayerByName(self.iface, "NotToxic")
        features = nottoxic_building.getFeatures()
        # iterate through features to get attributes to make strings
        for f in features:
            #summary.append((f.attribute()))
            info = ''
    ...
    def updateTable(self,values):
        ...
        self.tableVeryToxic.setColumnCount(4)
        self.tableVeryToxic.setHorizontalHeaderLabels(["RiskLevel","Substance","Danger","Capneed"])
        ...
        for i, item in enumerate(values):
            ...
            self.tableVeryToxic.setItem(i, 3, QtGui.QTableWidgetItem(str(item[3])))
        ...
        self.tableVeryToxic.horizontalHeader().setResizeMode(3, QtGui.QHeaderView.ResizeToContents)
        self.tableVeryToxic.horizontalHeader().setResizeMode(4, QtGui.QHeaderView.Stretch)
        self.tableVeryToxic.resizeRowsToContents()

    def updateTable2(self,values):
        # send this to the table
        self.tableMediumToxic.setColumnCount(4)
        self.tableMediumToxic.setHorizontalHeaderLabels(["RiskLevel","Substance","Danger","Capneed"])
        ...
        for i, item in enumerate(values):
            ...
            self.tableMediumToxic.setItem(i, 3, QtGui.QTableWidgetItem(str(item[3])))
        ...
        self.tableMediumToxic.horizontalHeader().setResizeMode(3, QtGui.QHeaderView.ResizeToContents)
        self.tableMediumToxic.horizontalHeader().setResizeMode(4, QtGui.QHeaderView.Stretch)
        self.tableMediumToxic.resizeRowsToContents()

    def updateTable1(self,values):
        # send this to the table
        self.tableNotToxic.setColumnCount(4)
        self.tableNotToxic.setHorizontalHeaderLabels(["RiskLevel","Substance","Danger","Capneed"])
        self.tableNotToxic.setRowCount(len(values))
        for i, item in enumerate(values):
            # i is the table row, items mus tbe added as QTableWidgetItems
            self.tableNotToxic.setItem(i, 0, QtGui.QTableWidgetItem(str(item[0])))
            self.tableNotToxic.setItem(i, 1, QtGui.QTableWidgetItem(str(item[1])))
            self.tableNotToxic.setItem(i, 2, QtGui.QTableWidgetItem(str(item[2])))
            self.tableNotToxic.setItem(i, 3, QtGui.QTableWidgetItem(str(item[3])))
        self.tableNotToxic.horizontalHeader().setResizeMode(0, QtGui.QHeaderView.ResizeToContents)
        self.tableNotToxic.horizontalHeader().setResizeMode(1, QtGui.QHeaderView.ResizeToContents)
        self.tableNotToxic.horizontalHeader().setResizeMode(2, QtGui.QHeaderView.ResizeToContents)
        self.tableNotToxic.horizontalHeader().setResizeMode(3, QtGui.QHeaderView.ResizeToContents)
        self.tableNotToxic.horizontalHeader().setResizeMode(4, QtGui.QHeaderView.Stretch)
        self.tableNotToxic.resizeRowsToContents()
...
########################################################################################################################
    ...
    def buildNetwork(self):
        ...
        if self.network_layer:
            ...
            selected_layer = uf.getLegendLayerByName(self.iface, "Firestations")
...
########################################################################################################################
    ...
    def updatereportriskbuilding(self):
        ...
        rl = uf.getLegendLayerByName(self.iface, "NotToxic")
        numberNT = rl.featureCount()
        textnum = str(numberNT)
        textNT = "Low level of risk: %s buildings" % str(textnum)
        ...
        textMT = "Medium level of risk: %s buildings" % str(textnum)
        ...
        textVT = "High level of risk: %s buildings" % str(textnum)
        self.insertReportRiskBuilding(textVT)

        title2 = "Number of other risky buildings in the surrounding:"
    ...
    def trucksReport(self):
        ...
        Capneeds = uf.getFieldValues(rl, Capneedfield, null=True, selection=False)
        text_Capneeds = str(Capneeds[0][0])
        text_Capneeds2 = "Number of firetrucks needed was: %s" % str(text_Capneeds)
